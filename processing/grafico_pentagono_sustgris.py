...
def dibujar_pentagono(df, archivo_salida):
    ...
    for label, theta in zip(labels, angulos[:-1]):
        ...
        wrapped_label = textwrap.fill(label, width=10, break_long_words=False)
...
def poligono_sustgris(stats_folder):
    ...
    df1  = extraer_mediana_volumen_df(
               df0, ["Temporal","Parietal", "Occipital", "Frontal","Sustancia gris total"]
           )
    out  = Path(stats_folder) / "pentagono_sustgris.png"
