def generate_morphometric_report_epilepsia(dicom_dir, subjects_dir, base_control_path):
    ...
    #Cargar excel de volumetria para exportar asimetria de sustancia blanca e hipocoampo lateralizado
    archivo_xlsx = os.path.join(path_stats, 'volumetria.xlsx')
    df_hipo = pd.read_excel(archivo_xlsx, sheet_name='Volumenes')
    ...
    fila_hip_izq = df_hipo[df_hipo['Regiones_ESP'] == 'Hipocampo izquierdo'].copy()
    fila_hip_der = df_hipo[df_hipo['Regiones_ESP'] == 'Hipocampo derecho'].copy()
    fila_amig_izq = df_hipo[df_hipo['Regiones_ESP'] == 'Amígdala izquierda'].copy()
    fila_amig_der = df_hipo[df_hipo['Regiones_ESP'] == 'Amígdala derecha'].copy()
    extra_vol_rows = pd.DataFrame()

    # Preparar la fila para que coincida con el DataFrame de destino
    if not fila_hip_izq.empty and not fila_hip_der.empty:
    ...
    # Preparar filas de hipocampo y amígdala izquierdos/derechos para agregarlas a volúmenes
    def preparar_filas_volumetria(df_vol):
        if df_vol.empty:
            return pd.DataFrame()
        df_vol = df_vol.rename(columns={
            'Regiones_ESP': 'Measure:GrayVol',
            'Volumen_%VIT': 'Volrel% (sujeto)'
        })
        df_vol['Volumen_cm3'] = pd.to_numeric(df_vol['Volumen_cm3'], errors='coerce').apply(
            lambda x: "{:.2f}".format(float(x)) if pd.notna(x) else "-"
        )
        df_vol['Volrel% (sujeto)'] = df_vol['Volrel% (sujeto)'].apply(
            lambda x: formatear_porcentaje(x) if pd.notna(x) else "-"
        )
        df_vol['Rango_normal_ajustado_por_edad_según_%VIT'] = df_vol['Rango_normal_ajustado_por_edad_según_%VIT'].apply(formatear_rango)
        df_vol['Percentil (sujeto)'] = '0.00'
        return df_vol[['Measure:GrayVol', 'Volumen_cm3', 'Volrel% (sujeto)',
                       'Rango_normal_ajustado_por_edad_según_%VIT', 'Percentil (sujeto)']]

    hipo_rows = preparar_filas_volumetria(pd.concat([fila_hip_izq, fila_hip_der], ignore_index=True))
    amig_rows = preparar_filas_volumetria(pd.concat([fila_amig_izq, fila_amig_der], ignore_index=True))
    extra_vol_rows = pd.concat([hipo_rows, amig_rows], ignore_index=True)
    ...
    for categoria, regiones in categorias.items():
        ...
        columnas_volumen = ['Measure:GrayVol', 'Volumen_cm3', 'Volrel% (sujeto)',
                            'Rango_normal_ajustado_por_edad_según_%VIT', 'Percentil (sujeto)']

        # 3. Añadir hipocampo y amígdala izquierda/derecha a la categoría de volúmenes
        if categoria == 'Volúmenes cerebrales' and not extra_vol_rows.empty:
            datos_filtrados = pd.concat(
                [datos_filtrados[columnas_volumen], extra_vol_rows[columnas_volumen]],
                ignore_index=True
            )
        else:
            datos_filtrados = datos_filtrados[columnas_volumen]

        # 4. Guardar el DataFrame modificado
    ...
    def create_page_content(page_number):
        ...
        if page_number == 0:
            ...
            alto_max = 100
            parrafo_asimetrias.wrapOn(can, ancho_max, alto_max)
            parrafo_asimetrias.drawOn(can, 25, y_position-65)  # Ajusta x, y según sea necesario
