...
# ============================================================
REQUIRED_COLS = {"seccion", "id_corto", "descripcion", "valor", "unidad","volumen_relativo"}
...
def _read_model_csv(csv_path: Path,
    ...
    Lee un CSV de modelo en el formato largo estándar:
      columnas requeridas: seccion, id_corto, descripcion, valor, unidad, volumen relativo
    ...
    df["volumen_relativo"]    = pd.to_numeric(df["volumen_relativo"], errors="coerce")

    # Quitar valores no numéricos
    n_bad = df["volumen_relativo"].isna().sum()
    if n_bad:
        logger.warning(f"{csv_path.name}: {n_bad} fila(s) con 'volumen_relativo' no numérico -> se omiten.")
        df = df.dropna(subset=["volumen_relativo"]).copy()
...
def _compute_ratio_vs_ref(df_ref: pd.DataFrame, df_model: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    ref = df_ref[["seccion", "id_corto", "match_key", "valor", "volumen_relativo"]].rename(
        columns={"seccion": "seccion_ref", "volumen_relativo": "valor_ref"}
    ).copy()
    mdl = df_model[["seccion", "id_corto", "match_key", "valor","volumen_relativo"]].rename(
        columns={"seccion": "seccion_model", "id_corto": "id_corto_model", "volumen_relativo": "valor_modelo"}
    ).copy()
...
def graficar_modelos_vs_freesurfer(
    *,
    ...
    modelos = [
        ("fastsurfer", csv_fastsurfer, "scatter_fastsurfer3_rel_vs_FS.png"),
        ("synthseg",   csv_synthseg,   "scatter_synthseg3_rel_vs_FS.png"),
        ("clinical",   csv_clinical,   "scatter_clinical3_rel_vs_FS.png"),
    ]
...
if __name__ == "__main__":
    ...
    csv_fs   = Path("/home/mbudani/results/procesamiento/volbrain/FS/con_volumen_relativo.csv")
    csv_fast = Path("/home/mbudani/results/procesamiento/volbrain/fastsurfer/con_volumen_relativo.csv")
    csv_ss   = Path("/home/mbudani/results/procesamiento/volbrain/synthseg/con_volumen_relativo.csv")
    csv_cli  = Path("/home/mbudani/results/procesamiento/volbrain/clinical/con_volumen_relativo.csv")

    out_dir  = Path("/home/mbudani/results/graficos")  # la carpeta DEBE existir
...
#          csv_clinical=csv_cli,
#          output_dir=out_dir,
#          select=["eTIV", "Left-Thalamus", "Left-Caudate","ctx-lh-insula","Left-Putamen", "Left-Pallidum","Left-Hippocampus","Left-Amygdala","Left-Accumbens-area","Right-Thalamus","Right-Caudate","Right-Putamen","Right-Pallidum","Right-Hippocampus","Right-Amygdala","Right-Accumbens-area","ctx-lh-caudalanteriorcingulate","ctx-lh-entorhinal","ctx-lh-fusiform","ctx-lh-inferiorparietal","ctx-lh-lateralorbitofrontal","ctx-lh-medialorbitofrontal","ctx-lh-superiorfrontal","ctx-lh-superiortemporal","ctx-lh-insula,","ctx-rh-caudalanteriorcingulate","ctx-rh-entorhinal","ctx-rh-fusiform","ctx-rh-inferiorparietal","ctx-rh-lateralorbitofrontal","ctx-rh-medialorbitofrontal","ctx-rh-superiortemporal","ctx-rh-insula"],
#          units_filter=("mm^3",),
#          trendline=True,
#          trendline_mode="median"
#    )